...
async function fetchGitHub(url: string, token: string, asText = false, retries = 3): Promise<any> {
    ...
    for (let i = 0; i < retries; i++) {
        try {
            ...
            // Extract rate limits for logging
            ...
            if (response.status === 403 || response.status === 429) {
                ...
                return { ok: false, status: response.status, rateLimitRemaining: remaining };
            }

            return {
                ok: response.ok,
                status: response.status,
                data: asText ? await response.text() : await response.json(),
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        const fileRes = await fetchGitHub(`repos/${repoPath}/contents/.github/classroom/autograding.json`, githubToken);

        const currentRateLimit = fileRes.rateLimitRemaining;
        ...
        const commitRes = await fetchGitHub(`repos/${repoPath}/commits`, githubToken);
        ...
        const runsRes = await fetchGitHub(`repos/${repoPath}/actions/runs?head_sha=${latestCommit}`, githubToken);
        ...
        const jobsRes = await fetchGitHub(`repos/${repoPath}/actions/runs/${runId}/jobs`, githubToken);
        const autogradingJob = jobsRes.data.jobs?.find((j: { name: string }) => j.name.toLowerCase().includes('autograding'));
        ...
        // 5. Download Raw Logs
        const logRes = await fetchGitHub(`repos/${repoPath}/actions/jobs/${autogradingJob.id}/logs`, githubToken, true);
        ...
        const rawLog = logRes.data as string;
        ...
        logger.info({
            ...
            rateLimitRemaining: currentRateLimit || 'Unknown',
